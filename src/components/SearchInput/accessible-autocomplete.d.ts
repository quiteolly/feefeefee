declare module 'accessible-autocomplete/react' {
	...
	type SourceFunction = (query: string, populateResults: (values: unknown[]) => void) => unknown[];
	...
	interface AutocompleteTemplates {
		 ...
		 */
		inputValue(selectedSuggestion: unknown): unknown;
		 ...
		 */
		suggestion(suggestion: unknown): unknown;
	}
	...
	export interface AccessibleAutocompleteProps {
		 ...
		 * Default () => {}
		 */
		onConfirm?(confirmed: unknown): void;
	...
	export default function AccessibleAutocomplete(props: AccessibleAutocompleteProps): ReactElement | null;
}
